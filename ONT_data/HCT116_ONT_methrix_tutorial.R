...
BASE_DIR<-"/local3/nanopore_practical"
...
sample_info<-data.frame("Sample_Name"=gsub("-", ".", gsub(".hg19.full.bg", "", basename(bdg_files))), "Cell_line"="HCT116", "Sample_Group"=c("WT", "DKO"))
rownames(sample_info)<-sample_info$Sample_Name

meth <- methrix::read_bedgraphs(files = bdg_files, ref_cpgs = hg19_cpgs, coldata=sample_info,
        chr_idx = 1, start_idx = 2, strand_idx=NULL, beta_idx=4, cov_idx = 5,
...
# 1.1: QC report
methrix::methrix_report(meth = meth, output_dir = file.path(OUT_DIR, "methrix_report"))
...
saveRDS(sum_df, file=file.path(OUT_DIR, "ont_region_summaries.RDS"))
